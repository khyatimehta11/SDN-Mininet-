...
# controlb file of modified topology  that caontain
# host h1-->10.0.1.10\24,
# host h2-->10.0.2.10\24,
# host h3-->10.0.3.10\24,
# host h4-->10.0.2.10\24,
# server-->10.0.4.10\24,
# have 4 switches s1 s2 s3 s4 and one CoreSwitch

#********MAIN TASK in controller FILE**********

#To install IP matching flow rules on CoreSwitch
# S1, S2, S3, and S4 stay as a MAC learning switch
...
#check if source that generate packet is present in mac_port table then add the flows so that it will send packets to destination
    if packet.dst in self.mac_to_port:
        ...
        if self.connection.dpid != 12: # this condition will not allowed to insert rules to core switch as rules for  core switch is already defined in act_as_router function above
            self.connection.send(msg)
    else:
